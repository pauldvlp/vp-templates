...
const OPENAPI_IMPORT = ["import { swaggerUI } from '@hono/swagger-ui'", "import { openApiDocument } from './openapi'"].join('\n')
const OPENAPI_SETUP = ["app.get('/docs.json', (c) => c.json(openApiDocument))", "app.get('/docs', swaggerUI({ url: '/docs.json' }))"].join('\n')

const SERVEWEB_IMPORT = ["import { join } from 'node:path'", "import { serveStatic } from '@hono/node-server/serve-static'"].join('\n')
...
const SERVEWEB_STATIC = "app.use('/*', serveStatic({ root: join(import.meta.dirname, '..', '..', 'web', 'dist') }))"
...
export default defineTemplate<Options>({
  ...
  options: [
    ...
    { key: 'apiPort', type: 'string', message: 'Port the Hono api listens on', default: '3000', validate: validatePort },
    { key: 'webPort', type: 'string', message: 'Port the web dev server listens on', default: '5173', validate: validatePort },
    { key: 'openapi', type: 'boolean', message: 'Expose Swagger UI at /docs (OpenAPI doc generated from the Zod contracts)', default: false },
  ...
  async produce({ options }) {
    ...
    const files = readTree(TEMPLATE_DIR, (rel, content) => {
      ...
      if (rel === APP_TS) {
        out = options.openapi
          ? out.replace('// __OPENAPI_IMPORT__', OPENAPI_IMPORT).replace('// __OPENAPI_SETUP__', OPENAPI_SETUP)
          : out.replace(/^[ \t]*\/\/ __OPENAPI_[A-Z_]+__\n/gm, '')

        out = options.serveWeb
          ? out.replace('// __SERVEWEB_IMPORT__', SERVEWEB_IMPORT).replace('// __SERVEWEB_STATIC__', SERVEWEB_STATIC)
    ...
    return {
      ...
      suggestions: [
        ...
        ...(options.openapi ? [`Swagger UI: http://localhost:${options.apiPort}/docs (OpenAPI JSON at /docs.json).`] : []),
        ...(options.serveWeb ? [`--serveWeb is on: after \`vp run -r build\`, \`vp run @app/api#start\` serves the web app from the api.`] : []),
