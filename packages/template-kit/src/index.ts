...
export function validateScope(value: string): string | undefined {
  return /^@?[A-Za-z0-9][A-Za-z0-9._/-]*$/.test(value.trim())
    ? undefined
    : 'must be a package scope/name, e.g. @acme';
}
...
export function validateName(value: string): string | undefined {
  return /^@?[A-Za-z0-9][A-Za-z0-9._/-]*$/.test(value.trim())
    ? undefined
    : 'must be a package name, e.g. my-app or @acme/app';
}
...
export function validatePreset(value: string): string | undefined {
  return /^[A-Za-z0-9][A-Za-z0-9_-]*$/.test(value.trim())
    ? undefined
    : 'must be a style name or ui.shadcn.com code';
}
...
export function validateComponents(value: string): string | undefined {
  const parts = value
    ...
    .map((c) => c.trim())
    .filter(Boolean);
  return parts.every((c) => /^[a-z0-9][a-z0-9-]*$/.test(c))
    ? undefined
    : 'comma-separated component names, e.g. button,card,dialog';
}
...
function parseArgv(
  argv: string[],
  descriptors: readonly OptionDescriptor<any>[],
): { values: Record<string, string | boolean>; directory?: string } {
...
function resolveDefault<O>(
  d: OptionDescriptor<O>,
  options: Partial<O>,
  directory: string,
...
async function promptForOption(
  d: OptionDescriptor<any>,
  def: string | boolean | undefined,
): Promise<string | boolean | symbol> {
  if (d.type === 'boolean') {
    return prompts.confirm({
      message: d.message,
      initialValue: def === undefined ? false : Boolean(def),
    });
  }
  if (d.type === 'select') {
  ...
  return prompts.text({
    ...
    validate: d.validate
      ? (value) => (!value && fallback !== undefined ? undefined : d.validate!(value ?? ''))
      : undefined,
  }) as Promise<string | symbol>;
}
